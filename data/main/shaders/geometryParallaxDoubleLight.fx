...
PS_OUTPUT PS(VS_OUTPUT psIn) : COLOR
{
	...
	ambientAmount *= 0.2 * texNCombine2.b * (lightColor.r * 0.3 + lightColor.g * 0.5 + lightColor.b * 0.2 + 0.1);
	float3 materialColorModulated = lerp(materialColor.rgb, lightColor.rgb, saturate(ambientAmount * 0.2));
	...
	sunGlow = 0.55f * (sunGlow*sunGlow*sunGlow);
