...
PS_OUTPUT PS(VS_OUTPUT psIn) : COLOR
{
	...
	// MIPMAP COLORIZATION CODE
    float4 lodLevel;    
    asm 
    {
		getCompTexLOD2D lodLevel, parallaxShift, mapNormalHeightAmbientSampler
    };
    float3 lodColor;
    if (lodLevel.x >= 2) lodColor = float3(0, 0, 1);
    else if (lodLevel.x >= 1) lodColor = float3(0, 1, 0);
    else lodColor = float3(1, 0, 0);	
	...
	outStruct.rtColorEmi = float4(lodColor, texNCombine2.b + materialPropertiesM.w);
//	outStruct.rtColorEmi = float4(texNCombine1.rgb * materialColor * (materialPropertiesM.y * texCombine.a), (texNCombine2.b + materialPropertiesM.w) * 0.5);
    outStruct.rtNormalSpec = float4(texNormal2, texCombine.gb * materialPropertiesM.zx * 0.5f);
    return outStruct;
}
