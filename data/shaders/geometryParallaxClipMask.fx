...
float4x4 matrixP : WORLDVIEWPROJECTION;
...
PS_OUTPUT PS(VS_OUTPUT psIn) : COLOR
{
	...
	float3 maskedMaterialColor = lerp(float3(1,1,1), materialColor.rgb, texCombine.r);

	outStruct.rtColorEmi = float4(texNCombine1.rgb * maskedMaterialColor * (materialPropertiesM.y * texCombine.a), (texNCombine2.b + materialPropertiesM.w) * 0.5);
    outStruct.rtNormalSpec = float4(texNormal2, texCombine.gb * materialPropertiesM.zx);
    return outStruct;
}
