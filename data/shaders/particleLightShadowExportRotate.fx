...
void VS(int index : INDEX)
{
	...
	float2 valSinCos;
	sincos(velocityXYZFrame.x, valSinCos.x, valSinCos.y);
	valSinCos.x = -valSinCos.x;
	float4 expandDirection = float4(valSinCos.x * posScreenXYZRad.w, valSinCos.y * posScreenXYZRad.w, 0, posScreenXYZRad.w);
	...
	if (calculateLighting)
	{
		...
		float3 lightTrans2 = float3(dot(valSinCos, lightTrans.yx), dot(float2(valSinCos.y, -valSinCos.x), lightTrans.yx), lightTrans.z);
		//lightTrans2.y = dot(float2(valSinCos.y, -valSinCos.x), -lightAvgDirection.yx);
		//lightTrans2.x = dot(valSinCos, -lightAvgDirection.yx);
