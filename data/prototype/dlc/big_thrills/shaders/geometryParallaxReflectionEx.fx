    ...
    float2 inTex		: TEXCOORD0)
{
	...
	outStruct.positionWS = mul(float4(inPos, 1), (float4x3)matrixW);
    ...
    outStruct.normalWS    = normalize(mul(inNormal, (float3x3)matrixW));	// normal to world space
    outStruct.tangentWS   = normalize(mul(inTangent, (float3x3)matrixW));	// tangent to world space
    outStruct.binormalWS  = normalize(mul(inBinormal, (float3x3)matrixW));	// binormal to world space    
...
PS_OUTPUT PS(VS_OUTPUT psIn) : COLOR
{
	...
	float3 normalWS = normalize(psIn.normalWS);
	float3 tangentWS = normalize(psIn.tangentWS);
	float3 binormalWS = normalize(psIn.binormalWS);	
	...
	float3 texNormal3 = normalize((texNormal.x) * tangentWS - (texNormal.y) * binormalWS + texNormal.z * normalWS * materialParallax.w);	

	// Chrome / reflection
	float3 pixelDirectionWS = normalize(psIn.positionWS - cameraPosition);
	float pran = dot(texNormal3, pixelDirectionWS);
	float3 reflectionVector = pixelDirectionWS - texNormal3*(2*pran);
	float3 reflectionColor = texCUBE(mapReflectionSampler, reflectionVector).rgb;

	texNormal2.xy = (texNormal2.xy + 1) * 0.5;

	outStruct.rtColorEmi = float4(texNCombine1.rgb * materialColor.rgb * (materialPropertiesM.y * texCombine.a) + reflectionColor * 0.75f * texNCombine2.b, (materialPropertiesM.w + texNCombine2.b * dot(reflectionColor, reflectionColor) * 0.001f) * 0.5);
