...
float4 PS( float2 tex : VPOS ) : COLOR0
{
	...
	float2 expandTex = tex01 - normalize(tex01 - 0.5f) * saturate(userInput.x*0.5)*2.0 * 0.004f;
	...
	float heartBeat = saturate(userInput.x*0.5);

	float4 colorAdd = float4(0.004, -0.008, -0.008, 0) * heartBeat;
	percentage += 0.25f * heartBeat;
	percentage = saturate(percentage);
	
	return colorAdd + (1.0f - percentage) * (pixelColor * (1 - sampledColorParticles.a) + float4(sampledColorParticles.rgb, 1)) + percentage * sampledColorLastFrame;
