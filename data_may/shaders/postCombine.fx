...
float4 PS( float2 tex : VPOS ) : COLOR0
{
	...
	/*
	float focusBlur = abs(pixelDepthVS - 6.0) * 0.042;
	if (pixelDepthVS < 6.0)
	{
		focusBlur *= 5;
	}
	*/
	//float focusDistance = pixelDepthVS - dofParameters.x;
	//float focusBlur = abs(focusDistance);
	...
	pixelColor += sampledColorACC*sampledColorACC;
