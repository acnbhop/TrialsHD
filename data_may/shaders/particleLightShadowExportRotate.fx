...
void VS(int index : INDEX)
{
	...
	{
		lightAccDiff = float4(0.6, 0.6, 0.7, 0.6);		// Fake ambient
		...
		lightTrans.y = dot(float2(valSinCos.y, -valSinCos.x), -lightAvgDirection.yx);
		lightTrans.x = dot(valSinCos, -lightAvgDirection.yx);
