...
//
static uint32 _DecodeUTF16BE(const std::vector<uint8>& Data, size& Pos)
{
...
//
static void _EncodeUTF8(std::string& Out, uint32 CodePoint)
{
    if (CodePoint <= 0x7F)
    {
...
//
static uint32 _DecodeUTF8(const std::string& Str, size& Pos)
{
...
//
static void _EncodeUTF16BE(std::vector<uint8>& Out, uint32 CodePoint)
{
    if (CodePoint <= 0xFFFF)
    {
...
//
static std::string _UTF16BEToUTF8(const std::vector<uint8>& Data, size Offset, size CharCount)
{
    std::string Result;
    size Pos = Offset;
...
//
static uint16 _UTF8ToUTF16BE(const std::string& Str, std::vector<uint8>& Out)
{
    uint16 CharCount = 0;
    size Pos = 0;
