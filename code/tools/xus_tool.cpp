...
{
    std::cout << "=== THD XUS Tool ===\n";
    std::cout << "Usage:\n";
    std::cout << "  " << ProgramName << " <input.xus>                 Unpacks the XUS to a .xml file in the same directory\n";
    std::cout << "  " << ProgramName << " <input.xml>                 Packs the XML file back into a .xus file\n";
    std::cout << "  " << ProgramName << " --print <input.xus>         Prints the XUS summary to a .txt file\n";
    std::cout << "  " << ProgramName << " --print-console <input.xus> Prints the XUS summary to the console\n";
...
int main(int argc, char* argv[])
{
    ...
    if (Ext == ".xus")
    {
        ...
        {
            std::cout << "[i] Loading XUS for inspection: " << TargetPath << "\n";
            if (!XusData.Load(TargetPath))
            {
                return EXIT_FAILURE;
            }
            ...
            if (PrintToConsole) XusData.PrintSummary();
        ...
        else
        {
            ...
            std::cout << "[i] Unpacking XUS: " << TargetPath << "\n";
            if (XusData.Load(TargetPath))
            {
                if (XusData.ExportXML(OutputPath.string(), ExactLineEndings))
                {
    ...
    {
        std::filesystem::path OutputPath = InputPath;
        OutputPath.replace_extension(".xus");

        std::cout << "[i] Packing XML: " << TargetPath << "\n";
        if (XusData.ImportXML(TargetPath, ExactLineEndings))
        {
            if (XusData.Save(OutputPath.string()))
            {
    ...
    {
        std::cerr << "[-] Unsupported file extension: '" << Ext << "'. Must be .xus or .xml\n";
        return EXIT_FAILURE;
    }
